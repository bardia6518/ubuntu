...
1. `customers` — client/vendor directory
   ...
   - `balance` (numeric) — outstanding balance owed by this customer
   - `created_at` (timestamptz)

2. `invoices` — sales invoices
   - `id` (uuid, pk)
   - `invoice_number` (text, unique) — human readable invoice code
   - `customer_id` (uuid, fk -> customers, nullable, set null on delete)
   - `amount` (numeric)
   - `issue_date` (date)
   - `due_date` (date)
...
4. `calendar_events` — upcoming/past financial events (expected income/expense)
   - `id` (uuid, pk)
   - `title` (text)
